const MIN_VIDEO_SECONDS = 5;

export const VIDEO_PRICING_RULES = Object.freeze({
  'doubao-seedance-2-5-260628': Object.freeze({ type: 'per_second', creditsPerSecond: 100, minSeconds: MIN_VIDEO_SECONDS }),
  'doubao-seedance-2-0-fast-260128': Object.freeze({ type: 'per_second', creditsPerSecond: 80, minSeconds: MIN_VIDEO_SECONDS }),
  'doubao-seedance-2-0-mini-260615': Object.freeze({ type: 'per_second', creditsPerSecond: 40, minSeconds: MIN_VIDEO_SECONDS }),
  'MiniMax-Hailuo-2.3-Fast/768p/6s': Object.freeze({ type: 'per_run', credits: 200, seconds: 6 }),
  'MiniMax-Hailuo-2.3-Fast/768p/10s': Object.freeze({ type: 'per_run', credits: 300, seconds: 10 }),
  'MiniMax-Hailuo-2.3-Fast/1080p/6s': Object.freeze({ type: 'per_run', credits: 300, seconds: 6 }),
});

export const IMAGE_PRICING_RULES = Object.freeze({
  'gpt-image-2.5': Object.freeze({ type: 'per_run', credits: 20 }),
  'gpt-image-2.5-sunburst': Object.freeze({ type: 'per_run', credits: 50 }),
});

export const GENERATION_COSTS = Object.freeze({
  video: 0,
  image: 0,
});

function normalizedModel(value) {
  return String(value || '').trim();
}

function billableSeconds(value) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed === -1) return MIN_VIDEO_SECONDS;
  return Math.max(MIN_VIDEO_SECONDS, Math.ceil(parsed));
}

export function getVideoPricingRule(model) {
  return VIDEO_PRICING_RULES[normalizedModel(model)] || null;
}

export function getImagePricingRule(model) {
  return IMAGE_PRICING_RULES[normalizedModel(model)] || null;
}

export function calculateVideoGenerationCost(payload = {}) {
  const rule = getVideoPricingRule(payload.model);
  if (!rule) return null;
  if (rule.type === 'per_run') return rule.credits;
  return billableSeconds(payload.duration) * rule.creditsPerSecond;
}

export function calculateImageGenerationCost(payload = {}) {
  const rule = getImagePricingRule(payload.model);
  return rule?.credits ?? null;
}

export const RECHARGE_PLANS = Object.freeze({
  cny10: Object.freeze({ id: 'cny10', credits: 1000, amountCents: 1000 }),
  cny50: Object.freeze({ id: 'cny50', credits: 5000, amountCents: 5000 }),
  cny100: Object.freeze({ id: 'cny100', credits: 10000, amountCents: 10000 }),
});

export const MIN_CUSTOM_RECHARGE_AMOUNT_CENTS = 1000;
export const RECHARGE_CREDITS_PER_YUAN = 100;

function parseAmountCents(value) {
  const text = String(value ?? '').trim();
  if (!/^\d+(?:\.\d{1,2})?$/.test(text)) return null;
  const [yuanPart, centPart = ''] = text.split('.');
  const yuan = Number(yuanPart);
  const cents = Number(centPart.padEnd(2, '0') || 0);
  const amountCents = yuan * 100 + cents;
  return Number.isSafeInteger(amountCents) && amountCents > 0 ? amountCents : null;
}

function creditsForAmountCents(amountCents) {
  return Math.round((amountCents * RECHARGE_CREDITS_PER_YUAN) / 100);
}

export function getRechargePlan(planId, input = {}) {
  const id = String(planId || '').trim();
  if (id === 'custom') {
    const amountCents = parseAmountCents(input.amount ?? input.price);
    if (!amountCents || amountCents < MIN_CUSTOM_RECHARGE_AMOUNT_CENTS) return null;
    return {
      id: 'custom',
      credits: creditsForAmountCents(amountCents),
      amountCents,
    };
  }
  return RECHARGE_PLANS[id] || null;
}
